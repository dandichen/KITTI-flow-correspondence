...
match_path = '/mnt/scratch/DandiChen/keypoint/KITTI/grid/ORB-BF/patch-image'
...
for img in range(pair_num):
    ...
    # grid: patch_idx = patch_y_idx * (x_num - 1) + patch_x_idx
    ...
    for patch_idx in range(len(img_patch1)):
        # print ''
        ...
        # kp1, des1, kp2, des2 = orb.get_keypoint(np.array(img_patch1[patch_idx]), np.array(img_patch2[patch_idx]))

        # patch-image keypoint pairs
        kp1, des1, kp2, des2 = orb.get_keypoint(np.array(img_patch1[patch_idx]), img2)
        # vis_kp.vis_pt_pairs(img1, kp1, x_trans1[patch_idx], y_trans1[patch_idx], \
        #                     img2, kp2, x_trans2[patch_idx], y_trans2[patch_idx])
        ...
        #                                            str(patch_idx).zfill(4) + '.npz'))
        # out_img = vis_matchers.vis_matches(img1, kp1, x_trans1[patch_idx], y_trans1[patch_idx], \
        #                          img2, kp2, x_trans2[patch_idx], y_trans2[patch_idx], matches)
        # cv2.imwrite(os.path.join(match_path + '/' + str(img).zfill(6), str(patch_idx).zfill(4) + '.png'), out_img)

        # evaluation

        # # keypoint before matching
        # patch_x, patch_y = keypoint.grid2mask(kp1, x_trans1[patch_idx], y_trans1[patch_idx])
        #
        # img_x.extend(patch_x)
        # img_y.extend(patch_y)

        # keypoint after matching
        patch_x, patch_y = BF_matcher.grid2mask(kp1, x_trans1[patch_idx], y_trans1[patch_idx], matches)
