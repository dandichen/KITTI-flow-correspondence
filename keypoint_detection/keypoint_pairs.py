...
class KeypointPairList(object):
    ...
    def get_neighbor_score(self, best_len=10, threshold=0.02):
        pt1 = np.array([self.list1.pos_x, self.list1.pos_y]).transpose()
        pt2 = np.array([self.list2.pos_x, self.list2.pos_y]).transpose()
        pairwise_dis = cdist(pt1, pt2, 'euclidean')
        pairwise_dis = normalize_arr(pairwise_dis, np.max(pairwise_dis),
                                       np.amin(pairwise_dis))
        ...
        # matchIdx = np.zeros((self.list1.length, best_len), dtype=int)
        # for idx in range(self.list1.length):
        #     matchIdx[idx] = pairwise_dis[idx].argsort()[:best_len]

        # smallest distance
        matchIdx = np.argmin(pairwise_dis, axis=1).transpose()
        return matchIdx
